...
func TestSyncerBringupColdToStreaming(t *testing.T) {
	...
	defer rawTgt.Close(context.Background())

	// parent <- child, FK enforced on both sides.
	...
	// Pre-flight to get the topo-ordered components (the daemon's real path).
	...
	for _, tbl := range []string{"customers", "orders"} {
		cur, err := store.LoadCursor(ctx, "s1", "dst", engine.TableRef{Schema: "rc_it", Name: tbl})
		if err != nil {
